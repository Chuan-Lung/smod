...
class Module:
	...
	def do(self,ip):
		...
		if (result != None):
        	...
        	for i in range(1,10): # Total of 255 (legal) uid
			...
			c = connectToTarget(ip,self.options['RPORT'][0])
                	...
                	for x in range(0,32):       # Total of 127 (legal) function codes 256->16
                    		...
                    		if ans:
                        		...
                        		self.printLine("[+] Function Code "+ str(x) +" probably supported.",bcolors.OKGREEN)
                            		probablyfunccode.append( str(x) )
			with open( ip + '.csv', 'wb') as f:
				fieldnames = ['UID' , 'SUPPORT' , 'PROBABLY']
